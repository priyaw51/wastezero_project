...
const OpportunityList = () => {
    ...
    // Helper to get deterministic room ID between two users
    const getRoomId = (a, b) => {
        if (!a || !b) return '';
                                        ...
                                        <div className={`px-6 py-4 border-t ${isDarkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
                                            {user?.role === 'volunteer' ? (
                                                <div className="space-y-2">
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            const rid = getRoomId(user._id, opportunity.ngo_id);
                                                            navigate(`/chat/${rid}`);
                                                        }}
                                                        className={`w-full py-2 px-4 rounded-lg font-medium transition-colors ${isDarkMode ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-blue-500 text-white hover:bg-blue-600'}`}
                                                    >
                                                        Chat with NGO
                                                    </button>
                                                </div>
